    ...
    const unregisterInsert = editor.registerCommand<InsertTableCommandPayload>(
      ...
      ({ columns, rows, includeHeaders = true }) => {
        ...
        editor.update(() => {
          ...
          // Ensure there's always a paragraph after the table for easy escape
          const afterParagraph = $createParagraphNode();
          tableNode.insertAfter(afterParagraph);

          // Select first cell's paragraph
          const firstRow = tableNode.getChildren()[0];
          if (
            $isTableNode(tableNode) &&
            firstRow &&
            (firstRow as any).getChildren
          ) {
            const firstCell = (firstRow as any).getChildren()[0];
            if (firstCell && (firstCell as any).getChildren) {
              const firstParagraph = (firstCell as any).getChildren()[0];
              if (firstParagraph && (firstParagraph as any).selectStart) {
                (firstParagraph as any).selectStart();
              }
            }
          }
        });
    ...
    const unregisterArrowDown = editor.registerCommand(
      ...
      () => {
        ...
        if (tableNode) {
          ...
          // Find the current row node
          let currentRow = anchorNode;
          while (currentRow && currentRow.getParent() !== tableNode) {
            currentRow = currentRow.getParent()!;
          }

          if (currentRow === lastRow) {
            // Check if cursor is at the very end of the cell
